...
route("PUT", "/api/me/tags", async (req, res, params, body) => {
  ...
  if (openToNew !== undefined) {
    db.prepare("UPDATE users SET open_to_new = ? WHERE id = ?").run(openToNew ? 1 : 0, userId);
  }
...
route("GET", "/api/discover", async (req, res) => {
  ...
  // Cold-start mode: while there are only a handful of people on the whole
  // platform, gender/style filtering means almost nobody sees anybody. Below
  // this many total users, show everyone regardless of tags — age is still
  // respected. Kicks back in automatically once the user base grows.
  const COLD_START_USER_THRESHOLD = 50;
  const totalUsers = db.prepare("SELECT COUNT(*) as c FROM users").get().c;
  const coldStart = totalUsers < COLD_START_USER_THRESHOLD;
  ...
  candidates = candidates
    ...
    .filter((c) => {
      ...
      // "Offen für Neues" relaxes only *my own* filter (I want to see more),
      // it doesn't override what the other person is looking for.
      const passesMyTagFilter =
        coldStart || me.open_to_new || mySeeking.length === 0 || overlaps(mySeeking, cIdentityIds);
      const passesTheirTagFilter = coldStart || c.seekingIds.length === 0 || overlaps(c.seekingIds, myIdentity);
